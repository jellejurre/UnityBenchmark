...
public class BlendShapeActiveCountBenchmark : BenchmarkTask1d
{
	public override string GetName()
	{
		return "BlendShapeActiveCount";
	}

	public override string GetDescription()
	{
		return "Benchmarks Active blendshape count on a 2^16 triangle mesh with all vertices impacted";
	}
	...
	public override GameObject PrepareIteration1d(GameObject prefab, int value)
	{
		...
		for (int j = 0; j < 25; j++)
		{
			GameObject meshObj = Instantiate(MeshHelper.GetSquareMesh(sideLength, 1, value), parent.transform);
