...
public class BlendShapeSingleActiveCountBenchmark : BenchmarkTask1d
{
	public override string GetName()
	{
		return "BlendShapeSingleActiveCount";
	}

	public override string GetDescription()
	{
		return "Benchmarks Active blendshape count on a 2^16 triangle mesh with single vertices impacted";
	}
	...
	public override GameObject PrepareIteration1d(GameObject prefab, int value)
	{
		...
		for (int j = 0; j < 25; j++)
		{
			GameObject meshObj = Instantiate(MeshHelper.GetSquareMesh(sideLength, 1, value, true), parent.transform);
